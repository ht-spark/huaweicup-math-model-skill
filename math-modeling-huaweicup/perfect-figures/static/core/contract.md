...
For plotting tasks, first honor an explicit Python/R choice in the current request or a clearly language-specific input file/workflow. Save that backend as the user's default with `scripts/perfect_figures_backend.py set python` or `scripts/perfect_figures_backend.py set r`.

If the current request does not specify a backend, check the saved preference with `scripts/perfect_figures_backend.py get`. If it returns `python` or `r`, use that backend without asking again.
