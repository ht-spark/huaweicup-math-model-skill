#!/usr/bin/env python3
"""Read or write the user's default perfect-figures plotting backend."""
...
def config_path() -> Path:
    override = os.environ.get("PERFECT_FIGURES_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path("~/.config/perfect-figures/backend.json").expanduser()
...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Invalid perfect-figures config JSON at {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise SystemExit(f"Invalid perfect-figures config at {path}: expected object")
    return data
