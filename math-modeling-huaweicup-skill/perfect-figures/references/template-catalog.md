...
```bash
python perfect-figures/scripts/plot_templates.py volcano \
  --input results.csv --gene-col gene --effect-col log2fc --p-col padj \
  --output figures/volcano

python perfect-figures/scripts/plot_templates.py roc \
  --input roc.csv --fpr-col fpr --tpr-cols model_a,model_b \
  --output figures/roc

python perfect-figures/scripts/plot_templates.py dotplot \
